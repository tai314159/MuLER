        ...
        for i in range(len(sentences)):
            ...
            for sent in doc.sentences:
                ...
                for word in sent.words:
                    ...
                    if (word.feats is not None) and (word.feats.split("|")[0].split("=")[0] == mask.split("_")[0]):
                        ...
                        if len(mask.split("_")) > 1:
                            ...
                            if mask.split("_")[1] == "NOUN":  # mask only intersection of feat with NOUN
                            ...
                            if mask.split("_")[1] == "VERB":  # mask only intersection of feat with NOUN
                        ...
                        ######
                            ...
                            sentence_temp.append(mask)  # Append the feature itself (value for Gender/Number etc)
                            indices.append(i)
...
def choose_by_bleu(ref, candidates):
    ...
    index_max = list_bleu_per_candidate.index(max(list_bleu_per_candidate)) #index of candidate with max bleu score
...
def compute_scores(reference_input, candidate_input, DIR_OUT, parsed_model_ref, parsed_model_candidate,mask_list_path, mask_type=None):
    # note: parsed_model_ref & parsed_model_candidate are dictionaries, they are also saved they can be loaded with pickle
    # note: deleted save_pickle for files (because of run_all option in main function)

    #load mask list
    #note: this can also be generated, we need to decide
    ...
    ##ref_sentences = load_sentences(reference_input)  # not using this
    ##candidate_sentences = load_sentences(candidate_input)  # not using this
    #note: the reference_input & candidate_input are PRE_LOADED txt files
    ref_sentences = reference_input
    candidate_sentences = candidate_input
    ...
    bleu_not_masked, bleu_not_masked_list = score_sentence_bleu(reference_input, candidate_input, DIR_OUT, indices=None,
                                                                save_score=True)
    ...
    for mask in mask_list:
        ...
        print("len(reference_input)",len(reference_input))
        print("len(parsed_model_ref.keys())",len(parsed_model_ref.keys()))
        ...
        if len(indices) == 0:
            ...
            ########DELETE LATER#########
            if total_bleu < 0 :
                with open("/cs/snapless/oabend/tailin/MT/NEW/logs/"+mask_type+"_negative_bleu.txt","a") as outfile:

                    outfile.write("reference_masked: " + masked_ref[i] + "\n")
                    outfile.write("reference: " + reference_input[i] + "\n")
                    outfile.write("candidate_masked: " + masked_candidate[i] + "\n")
                    outfile.write("candidate: " + candidate_input[i] + "\n")
                    outfile.write("score_masked: " + str(score_masked) + "\n")
                    outfile.write("score_not_masked: " + str(score_not_masked) + "\n")
                    print("&" * 50)
            ##############################
    ...
    ##save_pickle(DIR_OUT + mask_type + "_results_bleu.txt", dict_results)
...
def parse_ref_candidate(DIR_OUT,  trg_lang, mask_type, reference, current_candidate, temp_filename):
    ...
    if not (CHECK_FOLDER_REF & CHECK_FOLDER_CAN):
        ...
        dict_parse_ref_mask = preprocess(reference, DIR_OUT,
                                         temp_filename + "_" + "_" + trg_lang + "_dict_parse_ref", lang=trg_lang,
...
def run_main(DIR_OUT, mask_list_path, ref_input, candidates_input,mask_type, trg_lang = "en", max_bleu_dict_path = None, run_all = False):

    """
    :param DIR_OUT:
    :param mask_list_path: path to txt file (each row is a mask) -- OR mask list
    :param ref: list of sentences OR path to reference file
    :param candidates: list of lists of sentences OR list of lists of candidate paths
    ...
    if run_all is True:
        ...
        for ind_candidate in range(len(candidates)):
            ...
            dict_parse_ref_mask, dict_parse_candidate_mask = parse_ref_candidate(DIR_OUT, trg_lang, mask_type, reference, current_candidate,temp_filename) #HERE
            ...
            list_results_dictionary[ind_candidate] = dict_results
        ...
        if max_bleu_dict_path is None:
            ...
            max_bleu_dict = choose_by_bleu(ref,candidates)
        ...
        dict_parse_ref_mask, dict_parse_candidate_mask = parse_ref_candidate(DIR_OUT,  trg_lang, mask_type,
                                                                             reference, current_candidate,temp_filename)
        # now we can use the parsed files instead of parsing at each iteration
        # compute scores (for each mask_type)

        dict_results = compute_scores(reference, current_candidate, DIR_OUT, dict_parse_ref_mask, dict_parse_candidate_mask,
                       mask_list_path, mask_type)
...
result = run_main(DIR_OUT, mask_list_path, ref, candidates,mask_type, trg_lang = "en", max_bleu_dict_path = None, run_all = False)
