...
def compute_scores(ref_input, candidate_input, DIR_OUT, parsed_model_ref,
    ...
    for mask in mask_list:
        ...
        ######delete later########
        for aa,bb,cc,dd in zip(masked_ref,ref_input,masked_candidate,candidate_input):
            print("masked_ref: ", aa)
            print("ref: ", bb)
            print("masked_candidate",cc)
            print("candidate",dd)
        ...
        print("indices",indices)
        print("*" * 100)
        print("len(masked_ref)", len(masked_ref))
        print("len(masked_candidate)",len(masked_candidate))
        print("len(ref_input)",len(ref_input))
        print("len(candidate_input)",len(candidate_input))
        print("*"*100)
        ...
        if len(indices) == 0:
            ...
            ########DELETE LATER#########
            if total_bleu < 0:
                with open("/cs/snapless/oabend/tailin/MT/NEW/outputs/negative_scores/" + mask_type + "_negative_rouge.txt", "a") as outfile:
                    outfile.write("reference_masked: " + masked_ref[i] + "\n")
                    outfile.write("reference: " + ref_input[i] + "\n")
...
mask_types = ["pos","ner"]
#NOTE: feat does not work atm for rouge
