#exp: mask X% oracle, (100-X)% anti-oracle, and compare between systems (Nov22)
#this is the hybrid scores experiment
#to view as a dataframe, use the code "stability_exp_results_to_df.py" on the outputs of this code
...
DIR_OUT_TEMP = "/cs/snapless/oabend/tailin/MT/NEW/outputs/stability_exp/"
...
def preprocess(txt_path, DIR_OUT, name_outfile, lang, model_type, mask_type=None, save=False):
    ...
    path = DIR_OUT + name_outfile + "_" + mask_type + ".txt"

    if os.path.isfile(path): #if we already parsed, load it and not parse again
        d = load_pickle(path)
        return d
    ...
    if mask_type == "pos":
        ...
        if save is True:
            save_pickle(path, dict_model_pickle)
...
def sentences_over_indices(sentences_list,indices):
    #get sentences restricted to the indices
    s = []
    for i in indices:
        s.append(sentences_list[i])
    return s


def bleu_all_scores(reference,candidate):

    r = word_tokenize(reference)
    c = word_tokenize(candidate)

    b = sentence_bleu([r], c)
    b1 = sentence_bleu([r], c,(1,0,0,0))
    b2 = sentence_bleu([r], c, (0, 1, 0, 0))
    b3 = sentence_bleu([r], c, (0, 0, 1, 0))
    b4 = sentence_bleu([r], c, (0, 0, 0, 1))
                ...
                for sent in doc.sentences:
                    ...
                    for word in sent.words:
                        if word.upos == mask:
                            ...
                            ner_doc_final.append([word_start,word_end,mask])
...
def mask_text_min(txt_path, parsed_model, mask, mask_type=None, mask_shape = None):
                ...
                for sent in doc.sentences:
                    ...
                    for word in sent.words:
                        if word.upos == mask:
                            ...
                            ner_doc_final.append([word_start, word_end, mask_shape])
                    ...
                    # deal with multiple masks (add serial nu. to the mask)
                    new_sentence1 = mask_multiple_words_version1(mask_shape, new_sentence)
                    masked_sentences.append(new_sentence1)
                    ###########################
    ...
    if mask_type == "ner":
        ...
        for i in range(len(sentences)):
            ...
            new_sentence = new_sentence.replace(mask, mask_shape)
...
##############################################################################################
#functions for the noun experiment (mask 50% of pos)
def mask_text_version0_exp(txt_path, parsed_model, mask, mask_type=None, split_list = None):
    """
    :return: masked sentnces + indices of masked sentences (ones containing the mask)
    changed for this experiment, mask only feat by split
    only for pos
    """
        ...
        for i in range(len(sentences)):
            ...
            if not sentences[i]:
                ...
                for sent in doc.sentences:
                    ...
                    ner_doc_final = []
                    for word in sent.words:
                        if word.upos == mask and word.text.startswith(tuple(split_list)):
...
def mask_text_version1_exp(txt_path, parsed_model, mask, mask_type=None,split_list = None):
    """
    :return: masked sentnces + indices of masked sentences (ones containing the mask)
    here mask is --> VERB1 VERB2...VERB3....
    """
...
def mask_text_min_exp(txt_path, parsed_model, mask, mask_type=None, mask_shape = None,split_list = None):
...
def mask_text_hyb(txt_path, parsed_model, mask, mask_type=None, mask_shape = None,split_list = None):

    """
    step 1: max mask (input for this function)
    step 2: hyb mask
    :param txt_path:
    :param parsed_model:
    :param mask:
    :param mask_type:
    :param mask_shape:
    :param split_list: complementary of split_list for max masking
    :return:
    """
        ...
        for i in range(len(sentences)):
            ...
            if not sentences[i]:
                ...
                for sent in doc.sentences:
                    ...
                    for word in sent.words:
                        if word.upos == mask and word.text.startswith(tuple(split_list)):
                            if not word.text.startswith(mask): #added for hybrid (to not mask again what was masked in the max masking process at step 1)
    ...
    if len(indices) == 0:
        ...
        if save_score == True:  ## I THINK I SHOULD DELETE THIS OPTION since I don't use it in the new code
            ...
            for i in indices:
                ...
                r = word_tokenize(reference[i])
                c = word_tokenize(candidate[i])

                score += sentence_bleu([r], c)
                bleu.append(sentence_bleu([r], c))
                temp_filename = "temp"
...
def muler_new(bleu_not_masked_list,bleu_masked_list,bleu_min_list):

    #print("len(bleu_not_masked_list),len(bleu_masked_list),len(bleu_min_list): ",len(bleu_not_masked_list),len(bleu_masked_list),len(bleu_min_list))
    ...
    for i in range(len(bleu_not_masked_list)):
        a = bleu_masked_list[i] - bleu_not_masked_list[i]
        b = bleu_masked_list[i] - bleu_min_list[i]
        #if b != 0 and a>0 and a<b:
        if b != 0  and a <= b and a>0:
            c = a/b
            score += c
        ########################
        if a<0:
        ...
        masked_check += bleu_masked_list[i]
        bleu_check += bleu_not_masked_list[i]
        #########################

    #print("score_a_neg (from muler_new function)",score_a_neg)
    score /= len(bleu_not_masked_list)

    masked_check /= len(bleu_masked_list)
    bleu_check /= len(bleu_not_masked_list)

    ######ADDED THIS#########
    #print("score masked, manual computation; muler_new func.", masked_check)
    #print("score not masked (bleu), manual computation; muler_new func.", bleu_check)
    #########################return

    return score

def muler_original(bleu_not_masked_list,bleu_masked_list,bleu_min_list):

    score = 0

    for i in range(len(bleu_not_masked_list)):
        ##print("bleu_not_masked_list[i],bleu_masked_list[i],bleu_min_list[i]: ",bleu_not_masked_list[i],bleu_masked_list[i],bleu_min_list[i])
        score += bleu_masked_list[i] - bleu_not_masked_list[i]
    score /= len(bleu_not_masked_list)
...
def compute_scores_exp(ref_input, candidate_input, DIR_OUT, parsed_model_ref, parsed_model_candidate, mask_list_path, filename_parse,
                   mask_type=None, score_type=None, mask_text_version = None, split_proportion = None):
    """
    new function for the stability experiment; added hybrid mask - 50-50 (or other split) max-min
    ...
    if mask_text_version is None:
        mask_text = mask_text_version0_exp
    if mask_text_version == "all":
        mask_text = mask_text_version1_exp
    ...
    ####added#####
    dict_sentence_muler1 = {} #if b!=0
    dict_sentence_muler2 = {} #if b!=0,a>0
    dict_max_bleu = {} #system level score masked (ONLY ON RELEVANT INDICES)
    dict_min_bleu = {} #system level score masked_min (ONLY ON RELEVANT INDICES)
    dict_bleu_system = {} #system level bleu (ONLY ON RELEVANT INDICES)
    dict_hyb = {}
    dict_bleu_indices = {} #bleu over indices (for each POS seperately)
    ##############
    ...
    dict_results[score_type + "_not_masked"] = bleu_not_masked  # ORIGINAL #this is not over the indices, it's the original bleu score of the sumbmission! it can be used to choose the file with the best bleu for example
    ...
    for mask in mask_list:
        ...
        ref_mask_list = get_mask_list(ref_input, parsed_model_ref, mask)
        _,l,res_ref = split_feature(ref_mask_list, split_proportion)
        print("ref: l",l)


        #can_mask_list = get_mask_list(ref_input, parsed_model_ref, mask) I think this is a mistake (fixed next row)
        can_mask_list = get_mask_list(candidate_input, parsed_model_candidate, mask)
        _, l, res_can = split_feature(can_mask_list, split_proportion)
        print("can: l",l)

        ###DELETE LATER###
        ref_counter = mask_list_counter(ref_input, parsed_model_ref, mask)
        can_counter = mask_list_counter(candidate_input, parsed_model_candidate, mask)

        outfile_counters.write("len(ref) " + str(len(ref_input)) + "\n")
        outfile_counters.write("ref_counter " + str(ref_counter) + "\n")
        outfile_counters.write("len(can) " + str(len(candidate_input)) + "\n")
        outfile_counters.write("can_counter " + str(can_counter) + "\n")
        ##############################################

        #max masking (no split) #asdfg
        masked_ref_max, indices_ref_max = mask_text(ref_input, parsed_model_ref, mask, mask_type, split_list=None)
        masked_candidate_max, indices_candidate_max = mask_text(candidate_input, parsed_model_candidate, mask, mask_type,
                                                        split_list=None) #split_list =None --> no split (just max masking)
        #min masking (no split)
        masked_ref_min, indices_ref_min = mask_text_min_exp(ref_input, parsed_model_ref, mask, mask_type,
                                                            mask_shape="X", split_list=None)
        masked_candidate_min, indices_candidate_min = mask_text_min_exp(candidate_input, parsed_model_candidate,
                                                                        mask, mask_type, mask_shape="Y",
                                                                        split_list=None)



        #NOTE: I did the same split for ref & can (if you want to separate, change res_ref to res_can in the masked candidata at step 1 & 2)
        #max masking for split proportion (e.g 0.5/0.4/0.3/etc.)
        masked_ref, indices_ref = mask_text(ref_input, parsed_model_ref, mask, mask_type,split_list=res_ref)
        masked_candidate, indices_candidate = mask_text(candidate_input, parsed_model_candidate, mask, mask_type,
                                                        split_list=res_ref)
        """#min masking for 50% pos
        masked_ref_min, indices_ref_min = mask_text_min_exp(ref_input, parsed_model_ref, mask, mask_type,
                                                            mask_shape="X", split_list=res_ref)
        masked_candidate_min, indices_candidate_min = mask_text_min_exp(candidate_input, parsed_model_candidate,
                                                                        mask, mask_type, mask_shape="Y",
                                                                        split_list=res_ref)""" #I think I don't need this

        #added #stability exp
        #step 1: max --> step 2: min (hyb)

        #step 2: use step 1 to complete hybrid masking
        split_list_ref_hyb = list(filter(lambda a: a not in res_ref, letters))
        split_list_can_hyb = list(filter(lambda a: a not in res_can, letters))

        #parse model for this (the masked_ref, masked_candidate)
        print("filename_parse",filename_parse)
        DIR_OUT_HYB = DIR_OUT + "parsed_files/"
        parsed_model_max_ref = parse_file(DIR_OUT_HYB,mask_type,masked_ref,filename_parse + mask + "_hyb_ref",trg_lang="en")
        parsed_model_max_can = parse_file(DIR_OUT_HYB,mask_type,masked_candidate,filename_parse + mask + "_hyb_can",trg_lang="en")

        #at this step: mask_text_hyb function takes the file which is masked with max (oracle) strategy for X% of the features (X = split_proportion) and then mask the rest with min (AO) strategy
        masked_ref_hyb, indices_ref_hyb = mask_text_hyb(masked_ref, parsed_model_max_ref, mask, mask_type, mask_shape="X",split_list=split_list_ref_hyb)
        masked_candidate_hyb, indices_candidate_hyb = mask_text_hyb(masked_candidate, parsed_model_max_can, mask,
                                                                mask_type, mask_shape="Y",
                                                                split_list=split_list_ref_hyb) #note: same split for ref & can (to change: change split_list parameter to split_list_can_hyb)
        """#check masking (delete later)
        with open(DIR_OUT_TEMP + filename_parse + ".txt","a") as fileout:
            print("mask",mask)

            print("max: masked_ref")
            for sen in masked_ref_max:
                print(sen +"\n")
            print("*"*30)

            print("max: masked_can")
            for sen in masked_candidate_max:
                print(sen + "\n")
            print("*" * 30)

            print("max: masked_ref")
            for sen in masked_ref:
                print(sen + "\n")
            print("*" * 30)

            print("min: masked_ref")
            for sen in masked_ref_min:
                print(sen + "\n")
            print("*" * 30)

            print("min: masked_can_min")
            for sen in masked_candidate_min:
                print(sen + "\n")
            print("*" * 30)

            print("max: masked_ref_hyb")
            for sen in masked_ref_hyb:
                print(sen + "\n")
            print("*" * 30)

            print("max: masked_can_hyb")
            for sen in masked_candidate_hyb:
                print(sen + "\n")
            print("@" * 100)"""

        ################################################################################################################################

        indices = indices_intersection(indices_ref_max, indices_candidate_max)

        indices_min = indices_intersection(indices_ref_min,indices_candidate_min)
        indices_hyb = indices_intersection(indices_ref_hyb,indices_candidate_hyb)

        indices1 = indices_intersection(indices,indices_min)
        indices_final = indices_intersection(indices1,indices_hyb)
        ...
        if len(indices) == 0:
            ...
            score_masked, bleu_masked_list = score_function(masked_ref_max, masked_candidate_max, DIR_OUT, indices,
                                                            save_score=False)
            #added
            score_masked_min, bleu_masked_min_list = score_function(masked_ref_min, masked_candidate_min, DIR_OUT, indices,
                                                            save_score=False)
            ...
            score_masked_hyb, bleu_masked_hyb_list = score_function(masked_ref_hyb, masked_candidate_hyb, DIR_OUT,
                                                                    indices,
                                                                    save_score=False) #hybrid

            #####################NEW SCORE######################


            print("original muler score : score_masked - score_not_masked", score_masked - score_not_masked)


            total_bleu = muler_new(bleu_not_masked_list,bleu_masked_list,bleu_masked_min_list)

            #second check #DELETE LATER #asdf
            score_check = 0
            counter_neg = 0
            counter_ab =0
            counter_b_zero = 0
            ...
            for i in range(len(bleu_not_masked_list)):
                ...
                a = bleu_masked_list[i] - bleu_not_masked_list[i] #muler
                b = bleu_masked_list[i] - bleu_masked_min_list[i] #max-min
                ...
                if a > b:
                    ##print("a>b -- max, min, bleu", bleu_masked_list[i],bleu_masked_min_list[i],bleu_not_masked_list[i])
                    counter_ab +=1
            ...
            score_check /= len(bleu_not_masked_list)

            #print("manual check (if b!=0) -- muler_new: ",mask,score_check) ###

            ###added another check -- system scores###
            a1 = score_masked - score_not_masked
            b1 = score_masked - score_masked_min
            if b1 == 0:
                #print("system score_masked - score_masked_min is zero")
                c1 = 0
            else:
                c1 = a1/b1
            #########################

            print("muler_new computed from system scores #1: ", c1) #will not be completely equal because we average here over more sentences (where sentence min - max is zero)
        ...
        dict_bleu[mask] = c1
        ###added###
        dict_sentence_muler1[mask] = score_check
        dict_sentence_muler2[mask] = total_bleu
        dict_max_bleu[mask] = score_masked
        dict_min_bleu[mask] = score_masked_min
        #dict_bleu_system[mask] = score_not_masked
        dict_bleu_indices[mask] = score_not_masked
        dict_hyb[mask] = score_masked_hyb
        ###########
    ...
    ###
    dict_results["max_bleu"] = dict_max_bleu
    dict_results["min_bleu"] = dict_min_bleu
    dict_results["bleu_indices"] = dict_bleu_indices
    dict_results["hyb_bleu"] = dict_hyb

    ###added###
    dict_results["sentence_muler_version1"] = dict_sentence_muler1
    dict_results["sentence_muler_version2"] = dict_sentence_muler2
...
def parse_file(DIR_OUT, mask_type, input, temp_filename, trg_lang="en"):
    ...
    print("**** path for hyb parsed file ****", DIR_OUT + temp_filename + "_dict_parse_" + mask_type + ".txt") ##delete later
...
def run_main_exp(DIR_OUT, mask_list_path, ref_input, candidates_input, mask_type, trg_lang="en",
             max_bleu_dict_path=None, run_all=False, score_type=None,mask_text_version=None,filename_parse = None,split_proportion=None):
    """
    :param DIR_OUT:
    :param mask_list_path: path to txt file (each row is a mask) -- OR mask list
    :param ref_input: list of sentences OR path to reference file
    ...
    if score_type == "bleu" or score_type is None:
        ...
        if run_all is True:
            ...
            for ind_candidate in range(len(candidates)):
                ...
                dict_results = compute_scores_exp(reference, current_candidate, DIR_OUT,
                                              dict_parse_ref_mask,
                                              dict_parse_candidate_mask,
                                              mask_list_path,filename_parse,mask_type,score_type,mask_text_version,split_proportion)
            ...
            dict_results = compute_scores_exp(reference, current_candidate, DIR_OUT, dict_parse_ref_mask,
                                          dict_parse_candidate_mask,
                                          mask_list_path, filename_parse,mask_type, score_type,mask_text_version,split_proportion)
...
def get_mask_list(txt_path,parsed_model,mask):

    #get list of all fetures (nouns/verbs/etc) in the txt (only pos)
    ...
    for i in range(len(sentences)):
        ...
        for sent in doc.sentences:
            for word in sent.words:
                if word.upos == mask:
                    feat_list.append(word.text)


    return feat_list


def mask_list_counter(txt_path,parsed_model,mask):
    ...
    counter_final = 0 #how many sentences has more than one of the mask

    for i in range(len(sentences)):
        doc = parsed_model[i]
        for sent in doc.sentences:
            counter_mask = 0  # temporary, delete later
            for word in sent.words:
                if word.upos == mask:
                    feat_list.append(word.text)
...
def split_feature(feat_list,split_proportion = None):
    """
    :param feat_list: mask list (e.g all nouns/verbs/etc.)
    ...
    #split_proportion --> 0.5 for 50%, etc.
    ...
    #create a dictionary of the splits
    for letter in letters:
        feat_letter = []
        for feat in feat_list:
            if feat.lower().startswith(letter):
                feat_letter.append(feat)
        dict_feat_split[letter] = feat_letter

        list_counts.append(len(feat_letter))

    #print("dict_feat_split: ",dict_feat_split)
    l=''
    sum = 0
    X = math.floor(len(feat_list) * split_proportion)

    print("X",X) ##

    for key in dict_feat_split.keys():
        sum += len(dict_feat_split[key])
        #print("key, sum", key, sum)
        if sum >= X :
            l = key
            temp = letters.index(l) + 1
            res = letters[:temp]

            return dict_feat_split,l,res
...
def compute_scores(ref_input, candidate_input, DIR_OUT, parsed_model_ref, parsed_model_candidate, mask_list_path,
    ...
    ##print("len(sentences)",len(bleu_not_masked_list))
    ...
    for mask in mask_list:
        ...
        #added
        masked_ref_min, indices_ref_min = mask_text_min(ref_input, parsed_model_ref, mask, mask_type, mask_shape = "X")
        masked_candidate_min, indices_candidate_min = mask_text_min(candidate_input, parsed_model_candidate,
                                                        mask, mask_type,mask_shape = "Y")
        ...
        if len(indices) == 0:
            ...
            if b1 == 0:
                ...
                c1 = a1/b1
                c2 = (score_masked/b1) - (score_not_masked/b1)
            #########################

            print("muler_new computed from system scores #1: ", c1) #will not be completely equal because we average here over more sentences (where sentence min - max is zero)

            if mask == "NOUN":
                dict_exp["noun_100%_original_muler"].append(score_masked - score_not_masked)
                dict_exp["noun_100%_new_muler"].append(total_bleu)
            if mask == "VERB":
                dict_exp["verb_100%_original_muler"].append(score_masked - score_not_masked)
                dict_exp["verb_100%_new_muler"].append(total_bleu)

            #print("muler_new computed from system scores #2: ", c2)

            #print("muler_new function (if b != 0  and a <= b and a>0)", total_bleu) ###

            #print("system min score, system max_score, system bleu score: ", score_masked_min,score_masked,score_not_masked)
    ...
    dict_results["max_bleu"] = dict_max_bleu
    dict_results["min_bleu"] = dict_min_bleu
    dict_results["bleu_indices"] = score_not_masked
...
split_prop = [0.5,0.3,0.4] #e.g, 0.3 means 0.3 Oracle masking (i.e max) and 70% Anti-Oracle masking (i.e min)
DIR_OUT_RESULTS = "/cs/snapless/oabend/tailin/MT/NEW/stability_results/"

with open("/cs/snapless/oabend/tailin/MT/NEW/codes/"+"stability_counters.txt","w") as outfile_counters: ###DELETE LATER###

    for split in split_prop:


        DIR_OUT_FINAL = DIR_OUT_RESULTS + str(split) + "/"
        make_folder(DIR_OUT_FINAL)


        for year, src_lang in zip(years,src_langs):

            if year == 20:
                ref_input = "/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt" + str(
                    year) + "/plain/references/newstest20" + str(year) + "-" + src_lang + trg_lang + "-ref.en.txt"

                DIR_CAN = "/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt" + str(
                    year) + "/plain/system-outputs/" + src_lang + "-" + trg_lang + "/"
                ...
                DIR_CAN = "/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt"+str(year)+"/plain/system-outputs/newstest20"+str(year)+"/"+src_lang+"-"+trg_lang+"/"

            dict_results_per_year = {} #new dict per year, combine later

            dict_results_per_year["year"] = year
            dict_results_per_year["langs"] = src_lang + "-" + trg_lang

            for filename in os.listdir(DIR_CAN):
                if filename != "newstest2018.JHU.5675.ru-en":
                    candidates_inputs = [[DIR_CAN + filename]]

                    #########DELETE LATER##########
                    outfile_counters.write("*"*50 + "\n")
                    outfile_counters.write(filename + "\n")
                    ###############################
                    ...
                    for mask_type in mask_types:
                        ...
                        for candidates_input in candidates_inputs:
                            ...
                            result = run_main_exp(DIR_OUT, mask_list_path, ref_input, candidates_input, mask_type,
                                                  trg_lang="en",
                                                  max_bleu_dict_path=None, run_all=True, score_type="bleu", mask_text_version=None,
                                                  filename_parse=filename_for_parse,split_proportion = split) #mask_text_version = None
                            ...
                            dict_results_per_year["submission"] = filename
                            dict_results_per_year["results"] = result

            # save checkpoints (per year)
            save_pickle(DIR_OUT_FINAL + str(year) + "_" + src_lang + "-" + trg_lang + "_stability_results.txt",dict_results_per_year)