#updated version of the muler score
...
def compute_scores_exp(ref_input, candidate_input, DIR_OUT, parsed_model_ref, parsed_model_candidate, mask_list_path,
    ...
    for mask in mask_list:
        ...
        _,l,res = split_feature(ref_mask_list, split_proportion=2)

        #print("len(ref_mask_list)",len(ref_mask_list))
        #print("l","res",l,res)
        ##############################################

        masked_ref, indices_ref = mask_text(ref_input, parsed_model_ref, mask, mask_type,split_list=res)
        masked_candidate, indices_candidate = mask_text(candidate_input, parsed_model_candidate,
                                                        mask, mask_type,split_list=res)

        """for ind_m in range(20):
            print("masked_ref[i]",masked_ref[ind_m])"""

        #print("len(masked_ref), masked_candidate",len(masked_ref),len(masked_candidate))

        #added
        masked_ref_min, indices_ref_min = mask_text_min_exp(ref_input, parsed_model_ref, mask, mask_type, mask_shape = "X", split_list=res)
        masked_candidate_min, indices_candidate_min = mask_text_min_exp(candidate_input, parsed_model_candidate,
                                                        mask, mask_type,mask_shape = "Y",split_list=res)
        ...
        if len(indices) == 0:
            ...
            #####################NEW SCORE######################
            ref_input_indices = sentences_over_indices(ref_input,indices)
            can_input_indices = sentences_over_indices(candidate_input, indices)
            masked_ref_min_indices = sentences_over_indices(masked_ref_min,indices)
            masked_candidate_min_indices = sentences_over_indices(masked_candidate_min, indices)
            masked_ref_indices = sentences_over_indices(masked_ref, indices)
            masked_candidate_indices = sentences_over_indices(masked_candidate, indices)

            #print("len(bleu_not_masked_list),len(bleu_masked_list),len(bleu_masked_min_list): ",len(bleu_not_masked_list),len(bleu_masked_list),len(bleu_masked_min_list))

            #total_bleu = score_masked - score_not_masked  # ORIGINAL MuLER
            ...
            #print("counter_neg",counter_neg)
            #print("counter_{a>b}", counter_ab)
            #print("counter_b_zero",counter_b_zero)
            #print("counter_{a=b}",counter_a_eq_b)
            #print("counter_neg_b",counter_neg_b)
            #print("counter_neg_c",counter_c)
            ...
            if mask == "NOUN":
                dict_exp["noun_50%_original_muler"].append(score_masked - score_not_masked)
                dict_exp["noun_50%_new_muler"].append(total_bleu)
            if mask == "VERB":
                dict_exp["verb_50%_original_muler"].append(score_masked - score_not_masked)
                dict_exp["verb_50%_new_muler"].append(total_bleu)
...
def split_feature(feat_list,split_proportion =2):

    #split_proportion --> 2 for 50%, etc.
    ...
    l=''
    sum = 0
    X = len(feat_list)/split_proportion
    #print("X",X)
        ...
        if len(indices) == 0:
            ...
            for i in range(len(bleu_not_masked_list)):
                ...
                if a < 0:
                    counter_neg += 1
                    """#print("a<0")
                    #print("ref (not masked): ",ref_input_indices[i])
                    #print("candidate (not masked): ",can_input_indices[i])
                    #print("ref masked max: ", masked_ref_indices[i])
                    #print("candidate masked max: ", masked_candidate_indices[i])
                    #print("*"*10)"""
                ...
                if a > b:
                    ...
                    #print("ref masked max: ", masked_ref_indices[i])
                    #print("candidate masked max: ", masked_candidate_indices[i])
                    #print("ref masked min: ", masked_ref_min_indices[i])
                    #print("candidate masked min: ", masked_candidate_min_indices[i])
                    #print("*"*10)"""
                ...
                if a == b:
                    ...
                    #print("*"*10)
                    #added bleu scores
                    #print("bleu not masked: ",bleu_all_scores(ref_input_indices[i],can_input_indices[i]))
                    #print("bleu masked max: ", bleu_all_scores(masked_ref_indices[i], masked_candidate_indices[i]))
                    #print("bleu masked min: ", bleu_all_scores(masked_ref_min_indices[i], masked_candidate_min_indices[i]))
                    #print("*" * 10)
                    #print("bleu_masked_list[i]",bleu_masked_list[i])
                    #print("bleu_not_masked_list[i]",bleu_not_masked_list[i])
                    #print("bleu_masked_min_list[i]",bleu_masked_min_list[i])"""

                if b == 0:
                    counter_b_zero +=1
                    """#print("b=0")
                    #print("bleu_masked_list[i],bleu_masked_min_list[i]: ",bleu_masked_list[i],bleu_masked_min_list[i])
                    #print("ref (not masked): ", ref_input_indices[i])
                    #print("candidate (not masked): ", can_input_indices[i])
                    #print("ref masked max: ", masked_ref_indices[i])
                    #print("candidate masked max: ", masked_candidate_indices[i])
...
candidates_inputs = [["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.Facebook_FAIR.6750.de-en"],["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.online-A.0.de-en"],["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.UCAM.6461.de-en"],["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.uedin.6749.de-en"],["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.online-A.0.de-en"], ["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.online-B.0.de-en"],["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.uedin.6749.de-en"]]

#candidates_inputs = [["/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/newstest2019.Facebook_FAIR.6750.de-en"]]
...
dict_exp = {}
dict_exp["system"] = []
dict_exp["noun_50%_original_muler"]=[]
dict_exp["noun_100%_original_muler"]=[]
dict_exp["verb_50%_original_muler"]=[]
dict_exp["verb_100%_original_muler"]=[]
dict_exp["noun_50%_new_muler"]=[]
dict_exp["noun_100%_new_muler"]=[]
dict_exp["verb_50%_new_muler"]=[]
dict_exp["verb_100%_new_muler"]=[]
...
for mask_type in mask_types:
    ...
    for candidates_input in candidates_inputs:
        ...
        dict_exp["system"].append(candidates_input[0].strip("/cs/snapless/oabend/borgr/SSMT/data/submissions/wmt19/plain/system-outputs/newstest2019/de-en/"))
        ...
        result = run_main_exp(DIR_OUT, mask_list_path, ref_input, candidates_input, mask_type, trg_lang="en",
                          max_bleu_dict_path=None, run_all=True, score_type="bleu", mask_text_version=None)

        print("*"*100)
        print("*"*100)

        #save_pickle(DIR_OUT_RESULTS +src_lang+"-"+trg_lang+"_"+mask_type+"_"+filename+".txt",result)
